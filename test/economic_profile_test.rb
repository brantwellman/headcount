...
class EconomicProfileTest < Minitest::Test
  ...
  def test_it_returns_estimated_median_household_income_in_year
    ep = EconomicProfile.new(@data)
    result1 = ep.median_household_income_in_year(2005)
    result2 = ep.median_household_income_in_year(2014)
    result3 = ep.median_household_income_in_year(2008)
  ...
  def test_it_throws_an_error_for_unknown_year_on_household_income_data
    ...
    assert_raises(UnknownDataError) {ep.median_household_income_in_year(2000)}
  end
  ...
  def test_returns_child_poverty_data_by_year
    ...
    def test_free_or_reduced_price_lunch_percentage
      ...
      assert_raises(UnknownDataError) { ep.free_or_reduced_price_lunch_total_in_year(2000)}
    end

    def test_free_or_reduced_price_lunch_by_number
      ep = EconomicProfile.new(@data)
      result = ep.free_or_reduced_price_lunch_total_in_year(2014)
